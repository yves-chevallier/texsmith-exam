...
| `exam.rules` / `rules` | list or mapping | `[]` | list of sentences or rule tokens, or `{preset, standard, prepend, append}` | Rules shown on the cover page; tokens and presets are resolved against the built-in library (`rules.yml`). See [Rule presets](#rule-presets). |
