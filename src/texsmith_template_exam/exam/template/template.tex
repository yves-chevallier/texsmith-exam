...
\AddToHook{env/tabular/begin}{\texsmithtabledepthinc}
\AddToHook{env/tabular/end}{\texsmithtabledepthdec}
...
\def\@fillin@relay[#1]{%
  ...
  \ifprintanswers
    ...
    \begingroup
      \setbox0 \hbox{\color@begingroup\fillin@ans\color@endgroup}%
      \ifdim\wd0 > #1\relax
        \hbox{\color@begingroup\fillin@ans\color@endgroup}%
      \else
        \hbox to #1{\color@begingroup\fillin@ans\hfil\color@endgroup}%
      \fi
    \endgroup
  \else
...
% Ensure code blocks start on a fresh line inside list items (no label overlap).
...
\makeatother
\qformat{\vbox{\hbox to \hsize{\textbf{\VAR{label_problem} \thequestion : \thequestiontitle}\hfill\ExamQuestionPoints}\vspace{0.8em}}}
...
\def\thedate{\VAR{date_formatted if date_formatted else "\\today"}}
...
\BLOCK{ if titlepage_minimal }
\def\@maketitle{%
  \logo[top=10mm,left=24mm,height=1.45cm]
  \begingroup
...
\footer{\ifthenelse{\equal{\VAR{next_page_advice|lower}}{true}}{\ifthenelse{\value{page}=\pageref{LastPage}}{\VAR{label_end_exam}}{\VAR{label_next_page}}}{}}{\VAR{label_page_of}}{\course}
