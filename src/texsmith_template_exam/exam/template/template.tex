...
\BLOCK{ set recto_name_raw = recto_name|default(false) }
\BLOCK{ if exam is defined and exam }
  \BLOCK{ set recto_name_raw = exam.get('recto-name', exam.get('recto_name', recto_name_raw)) }
\BLOCK{ endif }
\BLOCK{ set recto_name_enabled = recto_name_raw }
\BLOCK{ if recto_name_raw is string }
  \BLOCK{ set recto_name_enabled = recto_name_raw|lower|trim in ['1', 'true', 'yes', 'on'] }
\BLOCK{ endif }
...
% Name-entry field reused on the cover and (optionally) on recto pages.
\newlength{\examnameboxheight}
\setlength{\examnameboxheight}{0.5cm}
\newlength{\examnameboxwidth}
\setlength{\examnameboxwidth}{6.5cm}
\newlength{\examnameboxraise}
\setlength{\examnameboxraise}{1.25cm}
\newcommand{\ExamNameField}{%
  \begingroup
  \setlength{\fboxsep}{3pt}%
  \bfseries \VAR{label_name}~:\hspace{0.5em}%
  \fbox{\rule{0pt}{\examnameboxheight}\rule{\examnameboxwidth}{0pt}}%
  \endgroup
}
% Place the name field at the top-right of the text block, just above the
% header (i.e. above the problem number on running pages).
\newcommand{\ExamNameBoxTop}{%
  \AtTextUpperLeft{%
    \put(0pt,\examnameboxraise){%
      \makebox[\textwidth][r]{\ExamNameField}%
    }%
  }%
}
...
% The built-in HEIG-VD letterhead is drawn by the heiglogo fragment, which
% calls \logo without a size and therefore lands on the vintage default (18mm).
% That is the cover size: on a `minimal' title page the logo then reaches below
% the header rule and collides with the course line. The layout geometry this
% template resolved above is the intended one, so hand it to the fragment as
% the package default; \logo reads the defaults at shipout, so this holds
% wherever the fragment injected its call.
\BLOCK{ if logo_builtin and not logo_disabled and logo_height }
\makeatletter
\@ifpackageloaded{heiglogo}{%
  \heiglogoSetup{height=\VAR{logo_height}}%
}{}
\makeatother
\BLOCK{ endif }

% ---------------------------------------------------------------------------
% Contract macros (texsmith 0.8). The writers name a macro per construct and a
% fragment defines it; an exam restyles the ones it uses here, after the
% extra_packages slot above has loaded the fragments.
% ---------------------------------------------------------------------------

% tscode (ts-code): the exam wants a thin framed listing that hugs the part
% label above it, not a titled box floating in its own vertical space.
\makeatletter
\@ifpackageloaded{tcolorbox}{%
  \tcbset{/ts/code/.append style={%
    before skip=-0.95\baselineskip,
    after skip=0.4\baselineskip,
    top=0pt,
  }}%
}{}
\makeatother
\ifcsname tscode\endcsname
  \BeforeBeginEnvironment{tscode}{%
  ...
  \AfterEndEnvironment{tscode}{\par}
\fi

% tsdivider (ts-typesetting): a thematic break (---) is a page break in an
% exam. ts-typesetting already defines it that way, but the fragment is only
% loaded when the document needs it, so the exam defines it unconditionally.
\providecommand{\tsdivider}{\clearpage}
...
\def\@maketitle{%
  ...
  \BLOCK{ endif }
  \begingroup
  % The pull-up claws back the room the running head reserves. Taking 3mm
  % more on top of that left the title block riding under the logo, whose
  % bottom edge then all but touched the header rule; the Typst side clears
  % it by 2.8mm, so stop at what the running head actually costs.
  \vskip \dimexpr0pt-\headheight-\headsep-\topskip\relax%
  \noindent%
  \ifprintanswers
    \rlap{%
...
\BLOCK{ if recto_name_enabled and not solution }
% Repeat the name field on the recto of each sheet, i.e. on even physical
% pages (2, 4, 6, ...). A dedicated counter is used because exam.cls resets
% \value{page} after the cover, making its parity unreliable.
\newcount\examphyspage
\examphyspage=0
\AddToShipoutPictureFG{%
  \global\advance\examphyspage by 1\relax
  \ifodd\examphyspage\else\ExamNameBoxTop\fi
}
\BLOCK{ endif }
